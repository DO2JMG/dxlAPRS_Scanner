...
struct scanner_config {
  ...
  int minbandwidth = 5;
  int squelch = 65; 
  int timer_peaks = 60; // refresh frequency list after 60 seconds
...
int receivedata() {
  ...
  while (true) {
    ...
    if (buffer[0] == 'S' && buffer[1] == 'D' && buffer[2] == 'R' && buffer[3] == 'L' && readStatus > 0) {
      ...
      tool.debug("Elements : " + to_string(db.size()), false);
      ...
      for (auto i = db.begin(); i != db.end(); ++i) {
        ...
        if (db_noise_counter <= config.countermaxnoise) {
          ...
          if (config.verbous) tool.debug("db noise block : " + to_string(db_summary / config.countermaxnoise), false);
      ...
      for (long unsigned i = 0; i < (db.size()); i++) { // add peaks to the list
        if (db[i] > (avg_all + ((avg_all - avg_min)) - config.level)) {
          ...
          if (frequencyisonlist(iFrequency) == false) { // frequency is not on the list
            ...
            if (config.verbous) tool.debug("detected peak at " + to_string(iFrequency) + "hz db = " + to_string(db[i]), false);
          }
        }
      } 
...
int getpeaks() {
  ...
  while (true) {
    ...
    if (peaks.size() > 0) {
      ...
      i = 0;
      peaks_index_block = 0;   
       while ((i < frequency_count) && (frequencies.size() < 400)) {
        lowfrequency = peaks[peaks_index_block];
        if (config.verbous) tool.debug("smallest frequency " + to_string((int)lowfrequency), false);

        while((peaks_index_block < frequency_count-1) && ((peaks[peaks_index_block+1] - peaks[peaks_index_block]) < 5.0)) {
          peaks_index_block++;
        }

        if (config.verbous) tool.debug("greatest frequency "+to_string(peaks[peaks_index_block])+", at index "+to_string(peaks_index_block),false);
        ...
        if ((peaks_index_block - i) >= config.counterminpeaks)  // 
        {
          ...
          highfrequency = peaks[peaks_index_block-1];
          ...
          for (i = 0; i < blacklist.size(); i++) { 
            double rounded_frequency_b = std::round(((blacklist[i])/1000) * 100.0) / 100.0; 
          ...
          //if (config.verbous) tool.debug("center frequency " + to_string(frequency_list_new[frequency_count_new-1]), false);
        }
        peaks_index_block++;
        i = peaks_index_block;
